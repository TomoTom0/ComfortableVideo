...
(() => {
  ...
  var defaultSettings = {
    hoverDetectionTime: 2e3,
    controlsDisableTime: 3e3,
  ...
  async function loadSettings() {
    try {
      ...
      elements.hoverTimeValue.textContent = `${(settings.hoverDetectionTime / 1e3).toFixed(1)}${secondsUnit}`;
      elements.disableTime.value = settings.controlsDisableTime.toString();
      elements.disableTimeValue.textContent = `${(settings.controlsDisableTime / 1e3).toFixed(1)}${secondsUnit}`;
  ...
  function updateSliderValues() {
    elements.hoverTime.addEventListener("input", () => {
      const value = parseInt(elements.hoverTime.value) / 1e3;
    ...
    elements.disableTime.addEventListener("input", () => {
      const value = parseInt(elements.disableTime.value) / 1e3;
      const unit = chrome.i18n.getMessage("secondsUnit") || "\u79D2";
